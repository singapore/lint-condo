...
For our own project linting, we use the `latest` version of the image (so we can be the canary in a coal mine) but we also endeavour to tag releases regularly so you can "pin" to these if you like. We use the great service [Doppins](https://doppins.com/) to keep our `package.json` and `requirements.txt` up-to-date. Also note that the Docker Hub image is generated from automated builds of the `singapore/lint-condo` repository for your peace of mind.
...
Currently, `lint-condo` essentially echos the output of each linter as you would expect, followed by a summary of test results. An example of this summary is below:
...
2. You can add any custom commands/scripts of your own too, such as the above `sh test/colors.sh`. In our own projects we have multiple internal "linting" scripts that enforce project guidelines.
